...
import org.apache.flink.streaming.runtime.tasks.ExternallyInducedSourceAware;
...
public class StreamTaskExternallyInducedSourceInput<T> extends StreamTaskSourceInput<T> {
	...
	public StreamTaskExternallyInducedSourceInput(
			...
			ExternallyInducedSourceAware checkpointTriggeringHookProvider,
			int inputGateIndex,
			int inputIndex) {
		super(operator, inputGateIndex, inputIndex);
		this.checkpointTriggeringHook =
			checkpointTriggeringHookProvider.getCheckpointTriggeringHook(operator, getChannelInfos());
